...
def prepare_data(df: pl.DataFrame, min_ydstogo=1, max_ydstogo=10, min_situations=30):
    ...
    # Add era and flipped field position
...
def add_football_field(ax):
    # Slightly darker green for better line contrast
...
if __name__ == "__main__":
    ...
    # Custom non-green palette for eras (distinct, high contrast)
    era_palette = {
        '2000–2009': 'navy',
        '2010–2014': 'royalblue',
    ...
    sns.lineplot(
        ...
        y='go_rate',
        hue='season',
        palette='rocket',  # Purple → red/orange/yellow (no green, dark = older)
        linewidth=2.2,
        alpha=0.9,
        ax=axes[1],
        legend=False,
    ...
    # Season legend (more descriptive title)
    axes[1].legend(
        title='Season\n(Darker purple = earlier years → Lighter = recent/more aggressive)',
        loc='center left',
        bbox_to_anchor=(1.02, 0.5),
        ncol=2,
        frameon=True
    ...
    # Overall caption for extra context
    fig.text(0.5, 0.02,
             'Left side = team\'s own territory (historically low go-for-it rates)\n'
             'Right side = opponent\'s territory (traditionally higher rates)\n'
             'Recent eras/seasons push higher into own territory (leftward shift = increased aggression)',
